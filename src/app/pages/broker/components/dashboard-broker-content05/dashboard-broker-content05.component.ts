...
@Component({
  selector: 'app-dashboard-broker-content05',
  templateUrl: './dashboard-broker-content05.component.html',
  styleUrls: ['./dashboard-broker-content05.component.css']
})
export class DashboardBrokerContent05Component implements OnInit, OnDestroy {
  ...
  onFilterChange(): void {
    ...
    obs$ = this.userService.getClientesByBrokerId(this.user.id);
  ...
  get canCreateContratoFor(): (u: any) => boolean {
    return (u: any) => {
      ...
      if (role === 'BROKER') {
        return !!this.activeEnterprise.brokerCriarContrato;
      }
  ...
  get canCreateReleaseFor(): (u: any) => boolean {
    return (u: any) => {
      ...
      if (role === 'BROKER') {
        return !!this.activeEnterprise.brokerCriarRelease;
      }
