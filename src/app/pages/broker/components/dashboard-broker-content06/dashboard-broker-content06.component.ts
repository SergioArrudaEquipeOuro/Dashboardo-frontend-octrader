...
@Component({
  selector: 'app-dashboard-broker-content06',
  templateUrl: './dashboard-broker-content06.component.html',
  styleUrls: ['./dashboard-broker-content06.component.css']
})
export class DashboardBrokerContent06Component implements OnInit, OnChanges {

  @Input() user: any;                // broker logado (tem id e email)
  @Input() activeEnterprise: any;    // para checar permissões (botões excluir, etc)
  ...
  ngOnInit(): void {
    // se `user` já veio pronto, busca
    if (this.user?.id) this.refresh();
  }
  ...
  /** Busca bots de TODOS os clientes do broker logado (user.id) */
  refresh() {
    if (!this.user?.id && !this.user?.email) {
      this.errorMsg = `Broker inválido ou sem ID/E-mail. ${this.user?.email}`;
      return;
    }
    ...
    // Preferência: por ID do broker; fallback para e-mail, se necessário
    const obs = this.user?.id
      ? this.botService.getByBrokerId(this.user.id)
      : this.botService.getByBrokerEmail(this.user.email);
    ...
    obs.pipe(finalize(() => (this.loading = false)))
      .subscribe({
        ...
        error: (err) => {
          // tenta um fallback via e-mail automaticamente, se falhou por ID e há e-mail
          if (this.user?.id && this.user?.email) {
            this.botService.getByBrokerEmail(this.user.email)
              ...
              .subscribe({
                ...
                error: (err2) => {
                  this.errorMsg = err2?.error?.message || err2?.message || 'Falha ao carregar bots.';
                }
              });
          } else {
  ...
  private buildStatusList(list: Bot[]): string[] {
    ...
    if (s.size === 0) return ['WAIT', 'RUNNING', 'PAUSED', 'FINISHED', 'ERROR'];
  ...
  private inDateRange(v: any, from: string | null, to: string | null): boolean {
    ...
    if (!d) return !(from || to); // se não há filtro de data, passa
    if (from) {
  ...
  /** Habilita ações administrativas (excluir) */
  get role(): boolean {
    const role = this.user?.role;
    if (role === 'ROOT' || role === 'ADMINISTRADOR') return true;
    if (role === 'BROKER' && this.activeEnterprise?.brokerCreateBot === true) return true;
    return false;
  }
}
