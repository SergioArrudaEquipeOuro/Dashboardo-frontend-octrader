// src/app/components/dashboard-admin-content07/dashboard-admin-content07.component.ts
...
@Component({
  selector: 'app-dashboard-admin-content07',
  templateUrl: './dashboard-admin-content07.component.html',
  styleUrls: ['./dashboard-admin-content07.component.css']
})
export class DashboardAdminContent07Component implements OnInit {
  ...
  manageBrokers: any[] = [];   // brokers da equipe
  manageClientes: any[] = [];  // clientes da equipe

  manageSelectedBrokerId: number | null = null;
  manageDestinoBrokerId: number | null = null;

  manageToLink: Set<number> = new Set();     // clientes marcados para vincular ao broker selecionado
  manageFromBroker: Set<number> = new Set(); // clientes marcados (do broker selecionado) para desvincular/transferir

  manageFiltroDisponiveis = '';
  manageFiltroBroker = '';

  // mapa brokerId -> Set<clienteId> (clientes que ESTE broker possui, globalmente)
  brokerClientesMap: Record<number, Set<number>> = {};


  // fechar ao clicar fora / ESC / scroll / resize
  ...
  // listas filtradas (nome OU email, case-insensitive)
  ...
  carregarEquipes(): void {
    ...
    this.equipeSvc.listarEquipes().subscribe({
      next: (dados) => {
        console.log('Dados retornados pela API:', dados);
        // Ordena as equipes pelo ID em ordem crescente com verificação de tipo
        this.equipes = (dados || []).sort((a, b) => (a?.id || 0) - (b?.id || 0));
      },
      error: (err) => {
        this.showAlert('Não foi possível carregar as equipes.', 'danger');
        console.error(err);
      },
  ...
  carregarEquipes2(): void {
    this.loadingList = true;
    this.equipeSvc.listarEquipes().subscribe({
      next: (dados) => {
        console.log('Dados retornados pela API:', dados); // Adicionado o console.log aqui
        this.equipes = dados.reverse() || [];
      },
      error: (err) => { this.showAlert('Não foi possível carregar as equipes.', 'danger'); console.error(err); },
      complete: () => this.loadingList = false
    });
  }
  ...
  criarEquipe(): void {
    ...
    this.equipeSvc.criarEquipe(payload).subscribe({
      next: () => { this.showAlert('Equipe criada com sucesso!', 'success'); this.form.reset(); if (this.equipes.length) this.carregarEquipes(); },
  ...
  salvarEdicao(): void {
    ...
    this.equipeSvc.atualizarEquipe(this.updatingId, payload).subscribe({
      next: () => { this.showAlert('Equipe atualizada!', 'success'); this.updatingId = null; this.form.reset(); this.carregarEquipes(); },
      error: (err) => { this.showAlert('Erro ao atualizar a equipe.', 'danger'); console.error(err); }
    });
  }

  cancelarEdicao(): void { this.updatingId = null; this.form.reset(); }
  ...
  deletarEquipe(id?: number): void {
    ...
    this.equipeSvc.deletarEquipe(id).subscribe({
      next: () => { this.showAlert('Equipe excluída.', 'success'); this.carregarEquipes(); },
  ...
  removerBrokersSelecionados(equipeId: number): void {
    ...
    this.equipeSvc.removerBrokers(equipeId, ids).subscribe({
      next: () => {
        ...
        this.carregarBrokersSemEquipe(); // eles passam a aparecer em “disponíveis”
      },
  ...
  removerClientesSelecionados(equipeId: number): void {
    ...
    this.equipeSvc.removerClientes(equipeId, ids).subscribe({
      next: () => {
        ...
        this.selectedLinkedClienteIds.clear();
        this.carregarEquipes();
        this.carregarClientesSemEquipe(); // agora ficam disponíveis
      },
  ...
  toggleActionsMenu(id: number, event: MouseEvent): void {
    ...
    const gap = 6;           // espacinho entre botão e menu
    const menuWidth = 200;   // igual ao CSS (min-width)

    // calcula posição preferindo alinhar à direita do botão
    let left = rect.right - menuWidth;
    if (left < 8) left = 8;                        // margem esquerda
    let top = rect.bottom + gap;
    const maxTop = window.innerHeight - 12;        // limite inferior

    // se estourar a parte de baixo, tenta abrir pra cima
    if (top > maxTop) {
      top = Math.max(8, rect.top - gap - 260);     // 260 ~ altura típica do menu
    }
  ...
  confirmTransferir(): void {
    ...
    this.userSvc.transferirClientesBroker(this.manageSelectedBrokerId, this.manageDestinoBrokerId, ids).subscribe({
      next: () => {
        // tira do origem e adiciona no destino
  ...
  getDisponiveisFiltrados(): any[] {
    return this.manageClientes
      .filter(c => !this.isClientInAnyBroker(c.id))              // <<< só “livres”
  ...
  openManageClientesModal(e: Equipe): void {
    ...
    this.manageTab = 'link';           // <- começa em Vincular
    ...
    this.userSvc.listarBrokers().subscribe({
      next: (brokers) => {
        this.brokerClientesMap = {};
        (brokers || []).forEach((b: any) => {
          this.brokerClientesMap[b.id] = new Set((b.clientes || []).map((c: any) => c.id));
        });
      },
  ...
  get role(): boolean {
    ...
    if (role === 'ROOT' || role === 'ADMINISTRADOR') {
      return true;
    } else if (role === 'SUPORTE' && this.activeEnterprise.suporteEditarEquipe === true) {
      return true;
    } else if (role === 'FINANCEIRO' && this.activeEnterprise.financeiroEditarEquipe === true) {
      return true;
    } else if (role === 'MANAGER' && this.activeEnterprise.managerEditarEquipe === true) {
      return true;
    }
    return false;
  }
