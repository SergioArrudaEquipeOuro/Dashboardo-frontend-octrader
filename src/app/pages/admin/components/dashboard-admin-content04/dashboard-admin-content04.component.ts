...
@Component({
  selector: 'app-dashboard-admin-content04',
  templateUrl: './dashboard-admin-content04.component.html',
  styleUrls: ['./dashboard-admin-content04.component.css']
})
export class DashboardAdminContent04Component implements OnInit {
  ...
  get canDeleteContratoFor(): (u: any) => boolean {
    return (_u: any) => {
      ...
      const role =
        U.includes('ROOT') ? 'ROOT' :
          U.includes('ADMIN') ? 'ADMINISTRADOR' :
            U.includes('SUPORTE') ? 'SUPORTE' :
              U.includes('FINANCEIRO') ? 'FINANCEIRO' :
                U.includes('MANAGER') ? 'MANAGER' :
                  rawRole.toUpperCase();

      // logs de diagnóstico
      const ae = this.activeEnterprise;

      // ROOT / ADMIN sempre podem
      ...
      if (role === 'FINANCEIRO') {
        return !!ae.financeiroDeletearContrato;
      }
      if (role === 'SUPORTE') {
        return !!ae.suporteDeletearContrato;
      }
      if (role === 'MANAGER') {
        return !!ae.managerDeletearContrato;
      }
